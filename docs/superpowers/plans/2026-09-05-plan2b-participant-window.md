...
Amended after the final review: Task 1's pacer used a quarter-interval tolerance and re-anchored on every admitted frame, which left preset rates above roughly 0.8× the capture rate unpaced. The landed pacer keeps phase-preserving due times with a fixed 1 ms jitter tolerance (`PACER_JITTER_TOLERANCE`) and re-anchors only after a stall. Do not reintroduce the quarter-interval formula from the Task 1 text.
