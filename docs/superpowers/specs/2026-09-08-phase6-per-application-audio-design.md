...
- **The open dependency question (5.8) is answered.** The probe ran on the dev machine (Fedora 44, PipeWire 1.6, WirePlumber) and reported 25 of 25 clients carrying `application.process.binary`, so `/proc/<pid>/exe` stayed a fallback rather than the primary route. Two things the probe surfaced: a client that reaches the daemon through `pipewire-pulse` carries the *pulse daemon's* `pipewire.sec.pid`, so the `/proc` fallback can name `pipewire-pulse` rather than the application; and a node's own properties often lack the binary while its client carries it, which is why identity is resolved through the client global.
...
- **Enumeration (5.2).** The listing feeds its graph after the roundtrip rather than during it, because a Node global can be announced before the Client global it names. Enumeration also builds its own ephemeral `Graph` under `AudioSelection::All`, separate from the capture graph, because `Graph::sources()` reports only nodes that passed the selection predicate and the picker must list what is audible rather than what is selected.
- **The two PipeWire paths assume opposite global orderings (5.2).** `list()` defers feeding its graph until after the roundtrip specifically against a node arriving before the Client global that names it. `run()` does the opposite: it classifies each node inline as its global arrives and never re-evaluates one it verdicts `Unresolved` (or, under `Only`, `NotSelected` for want of a key) once the client that would have resolved it shows up later. The two cannot both be right — either the ordering hazard `list()` guards against is real, in which case a selected application whose globals arrive in that order goes silent for the life of the session, or `list()`'s comment overstates it. Manual verification's item 4 (launch a selected application after capture has started) is the check that settles it; it was not run this session (see below). If it fails, the remedy is re-evaluating unresolved nodes when their client global arrives, left to a follow-up rather than this branch.
