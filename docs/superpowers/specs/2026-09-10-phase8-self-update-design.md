...
- **The relaunch rejoins with `Room::rejoin_ticket()` (5.4).** `Room::ticket()` names this node as the way in, which is no way in for this node itself: gossip ignores a bootstrap peer equal to itself, so the relaunched process waited out `JOIN_TIMEOUT` and landed on the start screen. `rejoin_ticket()` names the other current members instead, each with the addresses the endpoint holds for it, since with `--no-relay` an id on its own resolves to nothing. Alone in the room it falls back to `ticket()` (the ticket format has no reading for an empty bootstrap list) and `Room::start` drops this node's own id from the gossip bootstrap, so that ticket reopens the same topic at once. The window takes the ticket in the `UpdateReady(Ok(()))` arm, not from `RoomView`, because building it queries the endpoint.
