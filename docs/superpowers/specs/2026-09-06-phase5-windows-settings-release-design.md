...
- **Launch built at open time (5.4, 7).** `App::open` builds `Launch::from_settings` from the current store and the retained command line flags each time a room opens, so a dialog Save applies to the next room without a restart. `Settings::load` also validates, so a saved relay URL that no longer parses follows the corrupt-file path (defaults, error on the start screen) instead of aborting the launch.
- **Release notes guard (5.7).** The publish job checks the tag's `%(objecttype)` is `tag` before reading its annotation; on a lightweight tag `%(contents)` prints the commit message, so the empty-file check alone could not catch it.
